---
title: "Estimating Chronic Disease Disparity Cook County"
...
INIL_Tracts_geom <- st_read("layers/IL_Tracts_geom.shp")
INIL_Places_geom <- st_read("layers/IL_Places_geom.shp")
INIL_Counties_geom <- st_read("layers/IL_Counties_geom.shp")
...
places_datadictionary %>%
  ...
  select(Category = category, 
         ...
         Variable = variable) %>%
  arrange(Category) %>%
  kbl(caption=paste0("PLACES Chronic Health Prevalance Categories, Variable Names and Descriptions")) %>%
...
studyarea_counties_acs <- acs2019_bytract %>% 
  ...
  summarize_at(.vars = vars(TotalPop:PopWht, TotPov_19:PopPov_19), .funs=c(sum="sum")) %>%
  left_join(studyarea_counties, by="geoid_county") %>%
  st_as_sf()
```

```{r study_area_map, fig.cap="Cook County Study Area Boundary"}
...
moran_stats %>% 
  arrange(-morani) %>% 
  left_join(places_datadictionary %>% select(type=name, Category = category, Description = short_desc), by="type") %>%
  select(Name = type,
         Description,
         Category,
         "Moran's I" = morani,
...
# filter(StudyArea == "Outside CCDPH") %>%
  ...
  select(geometry,
         ...
         smk_cp = CSMOKING_CrudePrev,
         mam_cp = MAMMOUSE_CrudePrev,
         cer_cp = CERVICAL_CrudePrev,
         col_cp = COLON_SCREEN_CrudePrev) %>%
...
# filter(StudyArea == "Outside CCDPH") %>%
  drop_na(ACCESS2_CrudePrev) %>%
  rename_with(~str_remove(.,"_CrudePrev")) %>%
  left_join(acs2019_bytract,by=c("geoid_tract"="GEOID")) %>%
  rename_with(~str_remove(.,"_")) %>%
  st_as_sf()

names(places_tracts_sa_vars) <- tolower(names(places_tracts_sa_vars))
...
places_tracts_sa_vars <- places_tracts_sa_vars %>% st_drop_geometry() %>% left_join(ccdph_tracts_districts, by=c("geoidtract"="gd_trct"))
...
variables_corr <- places_tracts_sa_vars %>% 
  select(`% below poverty` = pctpov19,
        `hypertension` = bphigh,
        `diabetes` = diabetes,
        `obesity` = obesity,
        `low physical activity` = lpa,
        `% black`=popblkpct,
        `% people of color` = popclrpct,
        `% latinx` = poplatpct,
        `% white` = popwhtpct,
        `% asian` = popasnpct) %>%
  drop_na()
